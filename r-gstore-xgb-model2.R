...
print("BASE MODEL with CV")
start <- proc.time()[3]  
#set.seed(1234)

set.seed(1234)
xgb <- xgb.train(params = param,data = dtrainM, trControl = xgb_trControl, nrounds = 1500,verbose = 1, watchlist = watchlistM, print_every_n = 100, early_stopping_rounds = 100, prediction = TRUE)
...
xgb
# plot all the trees
#xgb.plot.tree(model = xgb)  # needs HTM or render=FALSE
...
gammas <-c(0,5)
best_params_xgb <- 0
best_params_xgbcv <- 0                                ##############  IGNORE XGB.CV for Capstone purposes
best_score_xgb <- 0
best_score_xgbcv <- 0
best_count_xgb <- 0
best_count_xgbcv <- 0
...
#            xgbcvT <- xgb.cv( params = param, data = dtrainM, nrounds = 5000, nfold = 5, model = T, showsd = T, stratified = T, print_every_n = 500, early_stopping_rounds = 100, prediction = TRUE, maximize = F)
            xgbT <- xgb.train(params = param,data = dtrainM, trControl = xgb_trControl, nrounds = 1500 ,verbose = 0, watchlist = watchlistM, print_every_n = 500, early_stopping_rounds = 100, prediction = TRUE)
